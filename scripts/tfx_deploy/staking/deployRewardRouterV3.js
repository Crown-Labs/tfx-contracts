...
async function main() {
  ...
  const minRewardCompound = 0.01 * 10**18 // 0.01 = $15 ETH $1500
  ...
  const feeGlpTracker = await deployContract("RewardTracker", ["Fee GLP", "fGLP"], "fGLP (Fee GLP)", signer)
  const feeGlpDistributor = await deployContract("RewardDistributor", [nativeToken.address, feeGlpTracker.address], "feeGlpDistributor", signer)
  ...
  const rewardRouter = await deployContract("RewardRouterV3", [], "", signer)
