...
async function getArbValues() {
  ...
  const vestWithXlpOption = "0x9d8f6f6eE45275A5Ca3C6f6269c5622b1F9ED515"
  ...
  return { esGmx, esGmxBatchSender, vestWithGmxOption, vestWithXlpOption, gmxVester, xlpVester }
}
...
async function getAvaxValues() {
  ...
  const vestWithXlpOption = "0x28863Dd19fb52DF38A9f2C6dfed40eeB996e3818"
...
async function main() {
  ...
  const { esGmx, esGmxBatchSender, vestWithGmxOption, vestWithXlpOption, gmxVester, xlpVester } = values
  ...
  const vestWithXlpAccounts = []
  const vestWithXlpAmounts = []
  ...
  for (let i = 0; i < txns.length; i++) {
    ...
    if (txn.To.toLowerCase() === vestWithXlpOption.toLowerCase()) {
      vestWithXlpAccounts.push(txn.From)
      vestWithXlpAmounts.push(amount)
      totalEsGmx = totalEsGmx.add(amount)
    }
  }

  console.log("vestWithGmxAccounts", vestWithGmxAccounts.length)
  console.log("vestWithXlpAccounts", vestWithXlpAccounts.length)
  ...
  if (shouldSendTxns) {
    ...
    if (vestWithXlpAccounts.length > 0) {
      await sendTxn(esGmxBatchSender.send(xlpVester.address, 320, vestWithXlpAccounts, vestWithXlpAmounts), "esGmxBatchSender.send(xlpVester)")
    }
  }
}
