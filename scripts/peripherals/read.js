...
async function readVaultTokenInfo(vault, tokens, usdxAmount) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    console.log("vault.usdxAmounts", (await vault.usdxAmounts(token)).toString())
    console.log("vault.getRedemptionAmount", (await vault.getRedemptionAmount(token, usdxAmount)).toString())
...
async function readFees(vault, weth, usdc) {
  ...
  const ethTargetAmount = await vault.getTargetUsdxAmount(weth.address);
  const usdcTargetAmount = await vault.getTargetUsdxAmount(usdc.address);
  console.log("ethTargetAmount", ethTargetAmount.toString())
  console.log("usdcTargetAmount", usdcTargetAmount.toString())

  const initialAmount0 = await vault.usdxAmounts(usdc.address)
...
async function readXlpManager(xlpManager) {
  console.log("xlpManager.cooldownDuration", (await xlpManager.cooldownDuration()).toString())
}
...
async function main() {
  ...
  startTime = Date.now()
  await xlpManager.getAums()
  console.log("xlpManager.getAums", Date.now() - startTime)
}
