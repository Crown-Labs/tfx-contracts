...
async function getTestnetValues() {
  ...
  return { vault, vaultPriceFeed, fastPriceFeed, router, tokenManager, mintReceiver, positionRouter, positionManager }
}
...
async function main() {
  ...
  const { vault, vaultPriceFeed, fastPriceFeed, router, tokenManager, mintReceiver, positionRouter, positionManager } = await getValues()
