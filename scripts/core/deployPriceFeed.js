...
async function deployPriceFeedTestnet() {
  ...
  //const fastPriceFeedGov = { address: getContractAddress("deployer") } // owner fastPriceFeed

  const updater1 = { address: getContractAddress("updater") }
  const keeper1 = { address: getContractAddress("keeper") }
  const updaters = [updater1.address, keeper1.address]

  const signers = [getContractAddress("deployer")]
  ...
  const fastPriceEvents = await deployContract("FastPriceEvents", [], "", signer)
  ...
  //const secondaryPriceFeed = await contractAt("FastPriceFeed", "0xEeDC45Fff97f597D9bdd603C4302d687F58572Bc")
