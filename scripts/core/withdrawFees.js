...
async function withdrawFeesBsc() {
  ...
  const usdx = await contractAt("Token", "0x85E76cbf4893c1fbcB34dCF1239A91CE2A4CF5a7")
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (vaultAmount.gt(balance)) {
      ...
      await sendTxn(balanceUpdater.updateBalance(vault.address, token.address, usdx.address, expandDecimals(1, 18)), `updateBalance ${i}`)
    }
