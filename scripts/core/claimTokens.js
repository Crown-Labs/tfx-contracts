...
async function main() {
  ...
  const usdxYieldTracker = await contractAt("YieldTracker", "0x0EF0Cf825B8e9F89A43FfD392664131cFB4cfA89")
  ...
  const busdxUsdxPair = { address: "0x7Fea0c6022D81EE17146324E4F55f6A02E138Dab" }
  const autoUsdxPair = { address: "0x0523FD5C53ea5419B4DAF656BC1b157dDFE3ce50" }

  const wbnbClaimableForXgmtPair = await xgmtYieldTracker.claimable(xgmtUsdxPair.address)
  console.log(`claimable: ${ethers.utils.formatUnits(wbnbClaimableForXgmtPair, 18)} WBNB`)
  await sendTxn(xgmt.recoverClaim(xgmtUsdxPair.address, receiver.address), "recoverClaim")

  const accounts = [gmtUsdxPair, xgmtUsdxPair, busdxUsdxPair, autoUsdxPair]

  for (let i = 0; i < accounts.length; i++) {
    const account = accounts[i]
    const claimable = await usdxYieldTracker.claimable(account.address)
    console.log(`claimable ${i}: ${ethers.utils.formatUnits(claimable, 18)} WBNB`)
    await sendTxn(usdx.recoverClaim(account.address, receiver.address), `recoverClaim ${i}`)
  }
}
