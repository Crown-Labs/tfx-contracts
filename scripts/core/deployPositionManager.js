...
async function getTestnetValues() {
  ...
  const vaultPositionController = await contractAt("VaultPositionController", "0xc006edAa3162A8b5840F9E28513627961C54f3a5")
  const timelock = await contractAt("Timelock", await vault.gov(), signer)
  const router = await contractAt("Router", "0x4849793282A5A0dD594b39a7516a2955904433a6", signer)
  const weth = await contractAt("WETH", tokens.nativeToken.address)
  const orderBook = await contractAt("OrderBook", "0xe2bd3230b1A2947a30Ebb095C4cF0aD3f1e87043")

  const orderKeeper = { address: "0x083B4acb59B0D102740cDA8de8f31cB603091043" }
  const liquidator = { address: "0x083B4acb59B0D102740cDA8de8f31cB603091043" }
  const referralStorage = { address: "0x083B4acb59B0D102740cDA8de8f31cB603091043" }

  const partnerContracts = []

  return { vault, vaultPositionController, timelock, router, weth, depositFee, orderBook, orderKeeper, liquidator, referralStorage, partnerContracts }
}
...
async function main() {
  const { vault, vaultPositionController, timelock, router, weth, depositFee, orderBook, orderKeeper, liquidator, referralStorage, partnerContracts } = await getValues()

  const positionManager = await deployContract("PositionManager", [vault.address, vaultPositionController.address, router.address, weth.address, depositFee, orderBook.address])
