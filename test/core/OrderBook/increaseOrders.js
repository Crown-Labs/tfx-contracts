...
describe("OrderBook, increase position orders", function () {
    ...
    */
    ...
    it("createIncreaseOrder, bad input", async () => {
        ...
        // instead revertedWith("OrderBook: insufficient collateral" with fulfillRequest
        // orderBook initialize minPurchseTokenAmountUsd = expandDecimals(5, 30) 
        await defaultCreateIncreaseOrder({
            path: [dai.address],
            amountIn: expandDecimals(4, 18) // amountIn < minPurchseTokenAmountUsd
        }), counter++
        ...
        expect(daiBalanceBefore, 'DAI balance').to.be.equal(daiBalanceAfter);
    });
    ...
    it("executeOrder, current price is invalid", async () => {
        ...
        for ([triggerPrice, isLong, collateralToken, triggerAboveThreshold, newBtcPrice, setPriceTwice] of [
            ...
            // "reset" BTC price
            await increaseBlocktime(provider, 20)
            await fulfillController.requestUpdatePrices()
            await xOracle.fulfillRequest([
                { tokenIndex: tokenIndexs.USDT, price: toXOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexs.BTC, price: toXOraclePrice(BTC_PRICE), lastUpdate: 0 },
    ...
    it("executeOrder, long, purchase token same as collateral", async () => {
        ...
        const position = positionWrapper(await vaultPositionController.getPosition(defaults.user.address, btc.address, btc.address, defaults.isLong));
    ...
    it("executOrder, 2 orders with the same position", async () => {
        ...
        let position = positionWrapper(await vaultPositionController.getPosition(defaults.user.address, btc.address, btc.address, defaults.isLong));
        expect(position.collateral).to.be.equal('59900000000000000000000000000000000');
    ...
    it("executeOrder, long, swap purchase token to collateral", async () => {
        ...
        const position = positionWrapper(await vaultPositionController.getPosition(user0.address, btc.address, btc.address, defaults.isLong));
    ...
    it("executeOrder, short, purchase token same as collateral", async () => {
        ...
        const position = positionWrapper(await vaultPositionController.getPosition(defaults.user.address, dai.address, btc.address, false));
    ...
    it("executeOrder, short, swap purchase token to collateral", async () => {
        ...
        const position = positionWrapper(await vaultPositionController.getPosition(user0.address, dai.address, btc.address, false));
    ...
    it("executeOrder, short, pay BNB, no swap", async () => {
        ...
        const position = positionWrapper(await vaultPositionController.getPosition(user0.address, dai.address, bnb.address, false));
