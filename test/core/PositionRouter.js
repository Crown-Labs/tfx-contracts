...
describe("PositionRouter", function () {
  ...
  beforeEach(async () => {
    ...
    // set position router
    await positionRouter.setFulfillController(fulfillController.address, wallet.address) // default feeReceiver = deployer
  ...
  it("increasePosition minOut long", async () => {
    ...
    it("validateExecution", async () => {
      ...
      // permanent price
      await vaultPriceFeed.setPriceSampleSpaceTime(24 * 60 * 60)
  ...
  it("createIncreasePosition, executeIncreasePosition, cancelIncreasePosition", async () => {
    ...
    position = await vaultPositionController.getPosition(user2.address, dai.address, bnb.address, false)
    expect(position[0]).eq(toUsd(6000)) // size
  ...
  it("createIncreasePositionETH, executeIncreasePosition, cancelIncreasePosition", async () => {
    ...
    position = await vaultPositionController.getPosition(user2.address, bnb.address, bnb.address, true)
    expect(position[0]).eq(toUsd(6000)) // size
  ...
  it("executeIncreasePositions, executeDecreasePositions", async () => {
    ...
    queueLengths = await positionRouter.getRequestQueueLengths()
    expect(queueLengths[0]).eq(7) // increasePositionRequestKeysStart
    expect(queueLengths[1]).eq(8) // increasePositionRequestKeys.length
    expect(queueLengths[2]).eq(7) // decreasePositionRequestKeysStart
    expect(queueLengths[3]).eq(7) // decreasePositionRequestKeys.length

    // remove keeper
    await positionRouter.setPositionKeeper(fulfillController.address, false)
    ...
    const key = await positionRouter.increasePositionRequestKeys(7);
    expect((await positionRouter.increasePositionRequests(key)).account).not.eq(AddressZero)
  })
})
