...
describe("Vault.increaseLongPosition", function () {
  ...
  it("increasePosition long validations", async () => {
    ...
    await expect(vaultPositionController.connect(user1).increasePosition(user0.address, btc.address, btc.address, 0, true))
      .to.be.revertedWith("Vault: invalid msg.sender")
    await expect(vaultPositionController.connect(user1).increasePosition(user0.address, btc.address, btc.address, 0, true, { gasPrice: "21000000000" }))
    ...
    await expect(vaultPositionController.connect(user1).increasePosition(user0.address, btc.address, bnb.address, 0, true))
      .to.be.revertedWith("Vault: mismatched tokens")
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, bnb.address, toUsd(1000), true))
      .to.be.revertedWith("Vault: mismatched tokens")
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, dai.address, dai.address, toUsd(1000), true))
      .to.be.revertedWith("Vault: _collateralToken must not be a stableToken")
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, toUsd(1000), true))
    ...
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, 0, true))
    ...
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, toUsd(500), true))
      .to.be.revertedWith("Vault: maxLeverage exceeded")

    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, toUsd(8), true))
  ...
  it("increasePosition long", async () => {
    ...
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, toUsd(118), true))
      .to.be.revertedWith("Vault: reserve exceeds pool")
    ...
    expect(await xlpManager.getAumInUsdx(true)).eq("48029860000000000000") // 48.02986
    expect(await xlpManager.getAumInUsdx(false)).eq("46858400000000000000") // 46.8584
    ...
    await expect(vaultPositionController.connect(user0).increasePosition(user0.address, btc.address, btc.address, toUsd(200), true))
      .to.be.revertedWith("Vault: reserve exceeds pool")
  ...
  it("increasePosition long aum", async () => {
    ...
    expect(await xlpManager.getAumInUsdx(true)).eq(expandDecimals(99700, 18))

    expect(await vault.feeReserves(btc.address)).eq("300000") // 0.003 BTC
    expect(await vault.usdxAmounts(btc.address)).eq(expandDecimals(99700, 18))
    ...
    expect(await vault.getRedemptionCollateralUsd(btc.address)).eq(toUsd(99700))
    expect(await xlpManager.getAumInUsdx(true)).eq(expandDecimals(99700, 18))
    expect(await xlpManager.getAumInUsdx(false)).eq(expandDecimals(99700, 18))
    ...
    await fulfillController.requestUpdatePrices()
    await xOracle.fulfillRequest([
        { tokenIndex: tokenIndexs.BTC, price: toXOraclePrice(150000), lastUpdate: 0 } 
    ], 0)
    ...
    expect(await xlpManager.getAumInUsdx(true)).eq(expandDecimals(134510, 18)) // 30080 + (1.4962-0.8)*150000
    expect(await xlpManager.getAumInUsdx(false)).eq(expandDecimals(134510, 18)) // 30080 + (1.4962-0.8)*150000
    ...
    await fulfillController.requestUpdatePrices()
    await xOracle.fulfillRequest([
        { tokenIndex: tokenIndexs.BTC, price: toXOraclePrice(75000), lastUpdate: 0 } 
    ], 0)
    ...
    expect(delta[1]).eq(toUsd(40000))
    expect(await xlpManager.getAumInUsdx(true)).eq(expandDecimals(82295, 18)) // 30080 + (1.4962-0.8)*75000
    expect(await xlpManager.getAumInUsdx(false)).eq(expandDecimals(64890, 18)) // 30080 + (1.4962-0.8)*50000
    ...
    expect(await xlpManager.getAumInUsdx(true)).eq("102295000500000000000000") // 102295.0005
    expect(await xlpManager.getAumInUsdx(false)).eq("68196667000000000000000") // 68196.667
