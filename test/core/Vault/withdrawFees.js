...
describe("Vault.withdrawFees", function () {
  ...
  it("withdrawFees", async () => {
    ...
    expect(await usdx.balanceOf(wallet.address)).eq(0)
    expect(await usdx.balanceOf(user1.address)).eq("269190000000000000000000") // 269,190 USDX, 810 fee
    expect(await vault.feeReserves(bnb.address)).eq("2700000000000000000") // 2.7, 900 * 0.3%
    expect(await vault.usdxAmounts(bnb.address)).eq("269190000000000000000000") // 269,190
    expect(await vault.poolAmounts(bnb.address)).eq("897300000000000000000") // 897.3
    expect(await usdx.totalSupply()).eq("269190000000000000000000")
    ...
    expect(await usdx.totalSupply()).eq("388830000000000000000000") // 388,830

    await btc.mint(user0.address, expandDecimals(2, 8))
    await btc.connect(user0).transfer(vault.address, expandDecimals(2, 8))

    await vault.connect(user0).buyUSDX(btc.address, user1.address)
    expect(await vault.usdxAmounts(btc.address)).eq("239280000000000000000000") // 239,280
    expect(await usdx.totalSupply()).eq("508470000000000000000000") // 508,470
    ...
    expect(await vault.usdxAmounts(bnb.address)).eq("329010000000000000000000") // 329,010
