...
describe("RewardRouterV3", function () {
  ...
  it("mintAndStakeGlp, unstakeAndRedeemGlp, compound", async () => {
    ...
    // expect(await feeGlpTracker.stakedAmounts(user1.address)).eq("598300000000000000000") // 598.3
    expect(await feeGlpTracker.stakedAmounts(user1.address)).gt("3091000000000000000000") // 598.3 + compound
    // expect(await stakedGlpTracker.stakedAmounts(user1.address)).eq("598300000000000000000")
    expect(await eth.balanceOf(user1.address)).eq("993676666666666666") // ~0.99
  })
return
  ...
  it("glp: signalTransfer, acceptTransfer", async () =>{
    // await gmx.setMinter(wallet.address, true)
  ...
  it("handleRewards", async () => {
    ...
    await timelock.signalSetHandler(glpManager.address, rewardRouterV3.address, true)

    await increaseTime(provider, 20)
    await mineBlock(provider)

    await timelock.setHandler(glpManager.address, rewardRouterV3.address, true)
    ...
    await rewardRouterV3.connect(user1).mintAndStakeGlp(
      bnb.address,
      expandDecimals(1, 18),
      expandDecimals(299, 18),
      expandDecimals(299, 18)
