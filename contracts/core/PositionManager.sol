...
contract PositionManager is BasePositionManager {
    ...
    function liquidatePosition(
        ...
        // request oracle
        bytes memory data = abi.encodeWithSignature("fulfillLiquidatePosition(address,address,address,bool,address)", _account, _collateralToken, _indexToken, _isLong, _feeReceiver);
        IFulfillController(fulfillController).requestOracle(data, msg.sender, "");
    }
    ...
    function _validateIncreaseOrder(address _account, uint256 _orderIndex) internal view returns (uint256) {
        ...
        (uint256 size, uint256 collateral, , , , , , ) = IVaultPositionController(vaultPositionController).getPosition(_account, _collateralToken, _indexToken, _isLong);
