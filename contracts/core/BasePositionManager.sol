...
contract BasePositionManager is IBasePositionManager, ReentrancyGuard, Governable {
    ...
    function _increasePosition(address _account, address _collateralToken, address _indexToken, uint256 _sizeDelta, bool _isLong, uint256 _price) internal {
        ...
        if (_isLong) {
            require(IVault(_vault).getMaxPrice(_indexToken, true) <= _price, "BasePositionManager: mark price higher than limit");
        } else {
            require(IVault(_vault).getMinPrice(_indexToken, true) >= _price, "BasePositionManager: mark price lower than limit");
        }
    ...
    function _shouldDeductFee(
        ...
        (uint256 size, uint256 collateral, , , , , , ) = IVaultPositionController(vaultPositionController).getPosition(_account, collateralToken, _indexToken, _isLong);
